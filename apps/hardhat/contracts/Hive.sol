...
abstract contract BeesContract {
...
// TODO: make every hive a DAO controlled by the bees
contract Hive is ERC721Enumerable, IERC721Receiver {
    ...
    function returnAllBees(uint256 _id) external {
        require(
            ...
            'only hive owner can return the bees'
        );
    ...
    function renderBees(uint256 _id) internal view returns (string memory) {
        ...
        for (uint8 i = 0; i < beesById[_id].length; i++) {
            uint16 blocksTraveled = uint16(
                (block.number - blockAdded[beesById[_id][i]]) % 256
            );
    ...
    function onERC721Received(
        ...
        require(
            beesById[hiveId].length < 256,
            'hive has reached the max limit of 255 bees.'
        );
